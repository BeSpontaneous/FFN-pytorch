...
import slowfast.utils.checkpoint_pbn as cu
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(
        ...
        input_H.append(inputs[0][:, :, :cfg.DATA.NUM_FRAMES_H//4, :])
        input_L.append(inputs[0][:, :, cfg.DATA.NUM_FRAMES_H//4:(cfg.DATA.NUM_FRAMES_H+cfg.DATA.NUM_FRAMES_L)//4, :])
        input_M.append(inputs[0][:, :, (cfg.DATA.NUM_FRAMES_H+cfg.DATA.NUM_FRAMES_L)//4:(cfg.DATA.NUM_FRAMES_H+cfg.DATA.NUM_FRAMES_L+cfg.DATA.NUM_FRAMES_M)//4, :])
        
        input_H.append(inputs[1][:, :, :cfg.DATA.NUM_FRAMES_H, :])
        input_L.append(inputs[1][:, :, cfg.DATA.NUM_FRAMES_H:cfg.DATA.NUM_FRAMES_H+cfg.DATA.NUM_FRAMES_L, :])
        input_M.append(inputs[1][:, :, cfg.DATA.NUM_FRAMES_H+cfg.DATA.NUM_FRAMES_L:cfg.DATA.NUM_FRAMES_H+cfg.DATA.NUM_FRAMES_L+cfg.DATA.NUM_FRAMES_M, :])
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.DETECTION.ENABLE:
                ...
                preds_L, preds_M, preds  = model(input_L, input_M, input_H, True, meta["boxes"])
            else:
                preds_L, preds_M, preds = model(input_L, input_M, input_H, True)
            ...
            loss_L = cfg.TRAIN.LAMBDA * nn.KLDivLoss(reduction='batchmean')(nn.LogSoftmax(dim=1)(preds_L), nn.Softmax(dim=1)(preds.detach()))
            loss_M = cfg.TRAIN.LAMBDA * nn.KLDivLoss(reduction='batchmean')(nn.LogSoftmax(dim=1)(preds_M), nn.Softmax(dim=1)(preds.detach()))
...
def eval_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            # Compute the predictions.
            preds = model(None, None, inputs, False, meta["boxes"])
            ...
            preds = model(None, None, inputs, False)
