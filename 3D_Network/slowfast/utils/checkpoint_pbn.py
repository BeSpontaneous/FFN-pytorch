...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        for key in caffe2_checkpoint["blobs"].keys():
            ...
            pretrained_state_dict[converted_key] = torch.tensor(
                        caffe2_checkpoint["blobs"][key]
                    ).clone()
        
        for k in ms.state_dict():
            if 'head' in k:
                continue
            elif (k.replace('_4', '') in pretrained_state_dict):
                state_dict[k] = pretrained_state_dict[k.replace('_4', '')]
                # ms.state_dict.update({k:pretrained_state_dict[k.replace('_4', '')]})
            elif (k.replace('_8', '') in pretrained_state_dict):
                state_dict[k] = pretrained_state_dict[k.replace('_8', '')]
                # ms.state_dict.update({k:pretrained_state_dict[k.replace('_8', '')]})
            elif (k.replace('_16', '') in pretrained_state_dict):
                state_dict[k] = pretrained_state_dict[k.replace('_16', '')]
                # ms.state_dict.update({k:pretrained_state_dict[k.replace('_16', '')]})
            elif (k in pretrained_state_dict):
...
def c2_normal_to_sub_bn(key, model_keys):
    ...
    """
    if "bn_4.running_" in key:
        if key in model_keys:
            return key

        new_key = key.replace("bn_4.running_", "bn_4.split_bn.running_")
        if new_key in model_keys:
            return new_key
    elif "bn_8.running_" in key:
        if key in model_keys:
            return key

        new_key = key.replace("bn_8.running_", "bn_8.split_bn.running_")
        if new_key in model_keys:
            return new_key
    elif "bn_16.running_" in key:
        if key in model_keys:
            return key

        new_key = key.replace("bn_16.running_", "bn_16.split_bn.running_")
        if new_key in model_keys:
            return new_key
    else:
        return key
