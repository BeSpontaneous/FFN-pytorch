...
class Sth_ffn(torch.utils.data.Dataset):
    """
    ...
    def get_seq_frames(self, index, temporal_sample_index):
        ...
        num_frames_H = self.cfg.DATA.NUM_FRAMES_H
        num_frames_M = self.cfg.DATA.NUM_FRAMES_M
        num_frames_L = self.cfg.DATA.NUM_FRAMES_L

        video_length = self._path_to_videos[index][1]

        seg_size = float(video_length - 1) / num_frames
        seg_size_L = float(video_length - 1) / num_frames_L
        seg_size_M = float(video_length - 1) / num_frames_M
        seg_size_H = float(video_length - 1) / num_frames_H
        seq = []
        # index from 1, must add 1
        if self.mode == "train":
            for i in range(num_frames_H):
                start = int(np.round(seg_size_H * i))
                end = int(np.round(seg_size_H * (i + 1)))
                seq.append(random.randint(start, end) + 1)
            for i in range(num_frames_L):
                start = int(np.round(seg_size_L * i))
                end = int(np.round(seg_size_L * (i + 1)))
                seq.append(random.randint(start, end) + 1)
            for i in range(num_frames_M):
                start = int(np.round(seg_size_M * i))
                end = int(np.round(seg_size_M * (i + 1)))
                seq.append(random.randint(start, end) + 1)
        else:
