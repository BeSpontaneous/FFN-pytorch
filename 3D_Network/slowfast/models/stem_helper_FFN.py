...
def get_stem_func(name):
    ...
    """
    trans_funcs = {"x3d_stem": X3DStem, "basic_stem": ResNetBasicStem_FFN}
    assert (
...
class VideoModelStem_FFN(nn.Module):
    """
    ...
    def __init__(
        ...
        """
        super(VideoModelStem_FFN, self).__init__()
    ...
    def forward(self, x_4, x_8, x_16, training=False):
        if training == True:
            assert (len(x_4) == self.num_pathways
        ), "Input tensor does not contain {} pathway".format(self.num_pathways)
        # use a new list, don't modify in-place the x list, which is bad for activation checkpointing.
            y_4 = []
            y_8 = []
            y_16 = []
            for pathway in range(len(x_4)):
                m = getattr(self, "pathway{}_stem".format(pathway))
                x4, x8, x16 = m(x_4[pathway], x_8[pathway], x_16[pathway], True)
                y_4.append(x4)
                y_8.append(x8)
                y_16.append(x16)
            return y_4, y_8, y_16
            ...
            if x_4 is not None:
                ...
                for pathway in range(len(x_4)):
                    m = getattr(self, "pathway{}_stem".format(pathway))
                    y.append(m(x_4[pathway], None, None, False))
            elif x_8 is not None:
                y = []
                for pathway in range(len(x_8)):
                    m = getattr(self, "pathway{}_stem".format(pathway))
                    y.append(m(None, x_8[pathway], None, False))
            else:
                y = []
                for pathway in range(len(x_16)):
                    m = getattr(self, "pathway{}_stem".format(pathway))
                    y.append(m(None, None, x_16[pathway], False))
            
            return y


class ResNetBasicStem_FFN(nn.Module):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            norm_module (nn.Module): nn.Module for the normalization layer. The
                default is nn.BatchNorm3d.
        """
        super(ResNetBasicStem_FFN, self).__init__()
        self.kernel = kernel
        self.stride = stride
        self.padding = padding
        self.inplace_relu = inplace_relu
    ...
    def forward(self, x_4, x_8, x_16, training=False):
        if training == True:
            ...
            x_4 = self.bn_4(x_4)
            x_4 = self.relu(x_4)
            x_4 = self.pool_layer(x_4)
            
            x_8 = self.conv(x_8)
            x_8 = self.bn_8(x_8)
            x_8 = self.relu(x_8)
            x_8 = self.pool_layer(x_8)
            
            x_16 = self.conv(x_16)
            x_16 = self.bn_16(x_16)
            x_16 = self.relu(x_16)
            x_16 = self.pool_layer(x_16)
